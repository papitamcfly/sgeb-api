...
export const cubaitorParcialValidator = vine.compile(
  vine.object({
    ...
    numPins: vine.number().min(1).max(16).optional(),
    hostIp: vine
      .string()
      .trim()
      .regex(/^((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.){3}(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/)
...
/**
 * Recalibración. `volumenCargadoMl` no está: para eso existe la recarga, que
 * además reanuda las órdenes pausadas.
 */
export const configParcialValidator = vine.compile(
  vine.object({
    caudalMlSeg: vine.number().min(0.01).max(999.99).optional(),
    pinGpio: vine.number().min(0).max(39).optional(),
  })
)
